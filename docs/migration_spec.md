...
1. 纵向可扩展圆角输入框：`正面（输入正面匹配词）`
2. 纵向可扩展圆角输入框：`负面（输入负面匹配词）`
3. `HStack(spacing: 12)`：
   - `yy（正面更快）`
   - `zz（负面更快）`
...
- 选择历史预设后填充四个字段。
- 选择预设后 `yy` 和 `zz` 禁用。
- 更新已有预设时只更新正面词、负面词和更新时间，不更新 `yy/zz`。
- 删除当前选中预设后解除选中状态。
...
4. `兼容正确率` / 动态值
5. `不兼容正确率` / 动态值
6. `当 {concept} 与正面词汇配对时你的反应速度/错误率` / 动态值
7. `当 {concept} 与负面词汇配对时你的反应速度/错误率` / 动态值
8. `D 值` / 动态值
...
|---:|---|---|---:|---|---|
| 1 | `Block 1：目标分类练习` | targetPractice | 8 | `其他` | `{concept}` |
| 2 | `Block 2：属性分类练习` | attributePractice | 8 | `正面` | `负面` |
| 3 | `Block 3：不兼容联合练习` | incompatiblePractice | 10 | `其他 + 正面` | `{concept} + 负面` |
| 4 | `Block 4：不兼容联合正式` | incompatibleFormal | 15 | `其他 + 正面` | `{concept} + 负面` |
| 5 | `Block 5：目标换位练习` | targetSwapPractice | 10 | `{concept}` | `其他` |
| 6 | `Block 6：兼容联合练习` | compatiblePractice | 10 | `{concept} + 正面` | `其他 + 负面` |
| 7 | `Block 7：兼容联合正式` | compatibleFormal | 15 | `{concept} + 正面` | `其他 + 负面` |

总题数：76。

注：原 Swift 源码把 Block 3/4 标为“兼容”、Block 6/7 标为“不兼容”，
但实际配对恰好相反。本迁移保留题目顺序与按键映射，只修正标题和
`BlockType`，使“目标词 + 正面词”作为兼容条件参与报告计算。
...
| 1 | `其他按 S，{concept} 按 J` |
| 2 | `正面按 S，负面按 J` |
| 3、4 | `其他和正面按 S，{concept} 和负面按 J` |
| 5 | `{concept} 按 S，其他按 J` |
| 6、7 | `{concept} 和正面按 S，其他和负面按 J` |
